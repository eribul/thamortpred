...
author: Anne Garland, PhD`*`^1,2,3^, Erik Bülow, MSc`*`^2,4^, Erik Lenguerrand, MSc^5^, Prof Ashley Blom, PhD^5,6^, Prof J. Mark Wilkinson, PhD^7^, Adrian Sayers, MSc^5^, Prof Ola Rolfson, PhD^2,4^, Prof Nils P. Hailer, PhD^1,2^
date: '`r Sys.Date()`'
...
**OBJECTIVES** To develop and validate a personalized model to predict 90-day mortality after total hip arthroplasty (THA).

**DESIGN** Prospective multi-national population-based study.

**SETTING** Swedish Hip Arthroplasty Registry (SHAR) data for model derivation and internal validation, and data from the National Joint Registry (NJR) for England and Wales for external validation.
...
**PARTICIPANTS** `r format(nrow(df), big.mark = ",")` patients with cemented total hip arthroplasty (THA) due to osteoarthritis recorded in the SHAR, and `r format(N_njr$tot, big.mark = ",")` similarly selected patients recorded in the NJR.


**MAIN OUTCOME MEASURES** Predicted probability of death for any reason within 90 days after primary THA, estimated using a bootstrap ranking procedure together with a least absolute shrinkage and selection operator (LASSO) logistic regression model and evaluated by areas under the curve (AUC).
