...
**OBJECTIVES** We developed and validated a statistical model to predict 90-day mortality after total hip arthroplasty (THA) due to osteoarthritis.

**DESIGN** A multi-national observational registry study with prospective data collection and data linkage. We used a bootstrap ranking procedure together with logistic LASSO-regression, evaluated by the area under the receiver operation characteristics curve (AUC), and calibration belt plots.

**SETTING** Patients operated `r paste(range(df$P_SurgYear), collapse = " - ")` and identified through the Swedish Hip Arthroplasty Register, including all private and public hospitals in Sweden, for model derivation and internal validation. Similar patients from England and Wales,  recorded in the National Joint Register, for external model validation.
...
**PARTICIPANTS** `r format(nrow(df), big.mark = ",")` patients with cemented total hip arthroplasty (THA) due to osteoarthritis, and ASA class I-III, for model derivation and internal validation `r shar_summary`. An additional `r format(N_njr$tot, big.mark = ",")` patients for external validation `r njr_summary`.



**MAIN OUTCOME MEASURES** Predicted probability of death due to any reason within 90 days after THA.
...
**RESULTS** A simple main effects model combining `r coefs_print_reduced`, with good internal discrimination `r get_auc("Reduced model")`, as well as externally `r get_auc("NJR (Reduced model)")`. This model was superior to traditional models based on the Charlson `r get_auc("Charlson")`, and the Elixhauser `r get_auc("Elixhauser")` co-morbidity indices. The model was well calibrated for predicted probabilities up to 5 %. A web calculator to aid clinical usage was published at `r shiny_url`.
