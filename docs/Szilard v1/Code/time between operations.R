...
anne_pred <- dplyr::mutate(anne_pred, ProtGrp =  factor(ProtGrp,
             ...
             dplyr::mutate(KlinGrp = relevel( ProtGrp, ref = 'cemented'))                                 
...
### ------------
# The time between the fist and second operation
dat <- dplyr::select(anne_pred, lpnr, OpNr, Sida, OppDat)
dat2 <- dplyr::filter(dat, OpNr == 2)
dat1 <- dplyr::filter(dat, lpnr %in% dat2$lpnr) %>%
        dplyr::filter( OpNr == 1)
        
dat12 <- merge(dat1, dat2, by = 'lpnr')   
dat12 <- dplyr::mutate(dat12, Time = OppDat.y - OppDat.x)
