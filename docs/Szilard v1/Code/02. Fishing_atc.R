...
############ -------- Read in the ATC data 

#load("Z:\\SHPR\\SZILARD\\Beslutstöd\\Output\\atc_codes2016-03-23.RData")
`%In%` <- function(x, table) fastmatch::fmatch(x, table, nomatch = 0L) > 0L
#atc_codes <- dplyr::filter(atc_codes, lpnr %In% anne_pred$lpnr)
#opdat <- dplyr::select(anne_pred, lpnr, OppDat)
#
#atc_codes <- merge(atc_codes, opdat, by = 'lpnr', all.x = TRUE)

#atc_codes <- dplyr::filter(atc_codes, (EDATUM < OppDat & EDATUM > OppDat-365))
...
colnames(anne_pred)[9:51] <- gsub(' ', '_', colnames(anne_pred)[9:51])
...
res <- matrix(NA, ncol = 6, nrow = 43)
rownames(res) <- colnames(anne_pred)[9:51]
colnames(res) <- c('OR', 'LCI', 'UCI', 'C-ind', 'c_low', 'c_upp')
               
### --------------------- Survival Analysis ---------------------------------###

varlist <- names(anne_pred)[9:51]
...
save.xlsx("atc90days_2017_02_28.xlsx",res)  
